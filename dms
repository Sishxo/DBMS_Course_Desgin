...
void myDBMS::openDataBase(string dataBaseName) {
	...
	string pathName = "D:\\李颖\\李颖大三上\\数据库原理课程设计文档202012\\DBMS\\" + dataBaseName;//拼接存储路径
	//检查数据库是否存在
...
void myDBMS::myCreateDataBase(string dataBaseName) {
	string pathName = "D:\\李颖\\李颖大三上\\数据库原理课程设计文档202012\\DBMS\\" + dataBaseName;
	//先判断是否有该数据库存在
...
//	cout<<"请输入主键:"<<endl;
	...
	if (0 != access(pathName.c_str(), 0)) {//如果没有这个表
		...
		for (int i = 0; i < colName.size(); i++)
            //以 列名;类型;大小; 的形式存入文件
            if(colName[i]=="primarykey") {string key=type[i];ptr->key=key;}
			else wrin += colName[i] + ";" + type[i] + ";" + to_string(size[i]) + ";";
        wrin +="primary key "+ptr->key;
		wrin += "\n";//换行
...
void myDBMS::myInsert(string tableName,string value) {
	...
	for (int i = 0; i < tab[pos]->type.size(); i++) {
		...
		if (tmp == "int") {
			int x; ss >> x;
			//char x;cin>>x;int a=atoi(x);
				if(x>=pow(10,tab[pos]->size[i]))
                {cout<<"输入溢出！";
                 break;}
			else {
	...
	fprintf(tab[pos]->fp, "%c", '\n');//换行
	fclose(tab[pos]->fp);
	if(flag1==1&&flag2==1) cout << "插入成功!" << endl;
	else cout << "插入失败!" << endl;
	//cout << "插入成功!" << endl;
}
